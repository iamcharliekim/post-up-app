...
class GamesListItem extends React.Component {
  ...
  state = {
    ...
    game: this.props.location.state
      ? this.props.location.state.gamesList[0]
      : this.props.game,
    ...
    GamesApiService.getAttendanceCount(this.state.game.id).then(rsvp => {
      ...
      GamesApiService.getGameAttendance(this.state.game.id).then(game => {
        if (game) {
          ...
          userIsAttending = game.attendance.find(
            user => user.attending_user === +game.user_id
          );
  ...
  formatAddressURI = () => {
    let geocodeAddress = `${this.state.game.game_street}${this.state.game
      .game_city}${this.state.game.game_state} ${this.state.game.game_zip}`;
  ...
  openGame = () => {
    ...
    if (!this.state.gamesPage) {
      let gamesList = this.context.games.filter(
        game => game.id === this.state.game.id
      );
  ...
  render() {
    ...
    rosterList = this.state.rosterList
      ? this.state.rosterList.map((username, i) => (
          <span key={i}>{username.username}</span>
        ))
      : null;

    let date = moment(this.state.game.game_date.split("T")[0]).format(
      "MMMM Do YYYY"
    );
    let time = moment(this.state.game.game_time, "HH:mm:ss").format("hh:mm A");

    let gameStreet = this.state.game.game_street;
    let gameCityStateZip = `${this.state.game.game_city} ${this.state.game
      .game_state} ${this.state.game.game_zip}`;
      ...
      <React.Fragment>
        ...
        >
          ...
          <div className={styles["address"]}>
            ...
            <a
              href={`https://www.google.com/maps/dir/?api=1&origin=${this
                .context.userCoords.lat},${this.context.userCoords
                .lng}&destination=${this.state.gameAddressString}`}
              target="_blank"
          ...
          <div
            className={
              this.state.gamesPage ? styles["rsvp-link"] : styles["rsvp"]
            }
          ...
          <div className={styles["rsvp-attending"]}>
            <button
              ...
              disabled={this.state.disableCheckInBtn}
            >
              {" "}
              Check-in
            </button>
            <button
              ...
              disabled={this.state.disableCheckOutBtn}
            >
              {" "}
              Check-out
            </button>
          ...
          {this.state.gamesPage ? (
            <CommentsBoard game_id={this.state.game.id} />
          ) : null}
        </div>
      </React.Fragment>
    );
